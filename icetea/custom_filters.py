"""
This module contains some custom filters I needed to implement for a specific
application. However I thought that including them in ``django-icetea`` would
be nice, since they can be useful to more people.
"""
...
def in_all_filter(data, definition, values):
    ...
    Basically performs a case sensitive search for all values in ``values``,
    with an ``AND`` operator in between.
...
def in_list_filter(data, definition, values):
    ...
    .. note::
        ...
        A smart way to find this is:
            ...
            need to search it for every: "value" (including the quotes), within ``values``.
    """
    field = definition[:-9]
